...
public class FirstPersonDrawer extends DefaultViewer {
	...
	MazePanel panel ; // the graphics object for the buffer image this class draws on
	 ...
	 * @param view_offset, only used to get viewx and viewy */
		...
		@Override
		public void redraw(MazePanel mazePanel, int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		// if notified by model that state has changed
		// Query model for parameters
		//dbg("viewer.redraw called");
		maze.panel.canvas = mazePanel.getBufferGraphics() ;
		this.rset = rset ;
		...
		maze.panel.setColor(Color.BLACK);
		maze.panel.setShader(new LinearGradient(0, 0, 0, 592, Color.BLACK, Color.RED, TileMode.CLAMP));
		
		maze.panel.fillRect(0, 0, Constants.VIEW_WIDTH, 592);
		maze.panel.clearShader(null);
		
		// draw dark gray background on upper halfx
	    //Initialize the BitmapShader with the Bitmap object and set the texture tile mode  
		maze.panel.setColor(Color.DKGRAY);
	    BitmapShader shader = new BitmapShader(Globals.bitmap1, TileMode.REPEAT, TileMode.REPEAT);
	    maze.panel.setShader(shader);
		
		maze.panel.fillRect(0, 592, Constants.VIEW_WIDTH, 592);
		maze.panel.clearShader(null);
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int x1i = x1;
		int xd = x2-x1;
		maze.panel.setColor(seg.col);
		//BitmapShader fillBMPshader2 = new BitmapShader(Globals.fillBMP2, TileMode.REPEAT, TileMode.REPEAT);
	    //maze.panel.setShader(fillBMPshader2); 
		boolean drawn = false;
