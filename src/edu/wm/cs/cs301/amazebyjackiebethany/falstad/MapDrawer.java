...
public class MapDrawer extends DefaultViewer {
	...
	static int view_width = Constants.VIEW_WIDTH;
	static int view_height = Constants.VIEW_HEIGHT;
	static int map_unit = 128;
	static int map_scale = 10 ;
	static int step_size = map_unit/4;
	static Cells mazecells ;
	static Cells seencells ; 
	static int[][] mazedists ;
	// width and height of map are chosen according to a user given skill level
	static int mazew ; // width 
	static int mazeh ; // height
	...
	final static int viewd_unscale(int x) {
		return x >> 16;
	}
	 ...
	 */
	public static void draw_map(MazePanel panel, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		panel.setColor(Color.WHITE);
		panel.clearShader(null);
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				panel.setColor(seencells.hasWallOnTop(x, y) ? Color.WHITE : Color.GRAY);
				...
				panel.setColor(seencells.hasWallOnLeft(x, y) ? Color.WHITE : Color.GRAY);
	 ...
	 * @param gc
	 */
	public static void draw_currentlocation(MazePanel panel, int view_dx, int view_dy) {
	 ...
	 * @param offy
	 */
	public static void draw_solution(MazePanel panel, int offx, int offy, int px, int py) {
		// check parameters:
...
//			dbg(" Parameter error: position out of bounds: (" + px + "," + py + ") for maze of size " + mazew + "," + mazeh) ;
	...
	private static int getDirectionIndexTowardsSolution(int x, int y, int d) {
		for (int n = 0; n < 4; n++) {
