		...
		for (x = 0; x != width; x++) {
			...
			while (y < height) {
				...
				sl.add(new Seg(x*Constants.MAP_UNIT, starty*Constants.MAP_UNIT,
						0, (y-starty)*Constants.MAP_UNIT, Distance.getDistance(x, starty), colchange));
			}
			y = 0;
			while (y < height) {
				// find the beginning of a segment
				...
				sl.add(new Seg((x+1)*Constants.MAP_UNIT, y*Constants.MAP_UNIT,
						0, (starty-y)*Constants.MAP_UNIT, Distance.getDistance(x, starty), colchange));
			}
		}
	}
	/**
		...
		for (y = 0; y != height; y++) {
			...
			while (x < width) {
				...
				sl.add(new Seg(x*Constants.MAP_UNIT, y*Constants.MAP_UNIT,
						(startx-x)*Constants.MAP_UNIT, 0, Distance.getDistance(startx, y), colchange));
			}
			...
			while (x < width) {
				...
				sl.add(new Seg(startx*Constants.MAP_UNIT, (y+1)*Constants.MAP_UNIT,
						(x-startx)*Constants.MAP_UNIT, 0, Distance.getDistance(startx, y), colchange));
			}
		}
	}
